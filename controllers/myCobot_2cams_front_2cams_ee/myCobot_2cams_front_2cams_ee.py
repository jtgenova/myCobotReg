...
cam_ee_1 = supervisor.getDevice('cam_ee_1')
cam_ee_2 = supervisor.getDevice('cam_ee_2')
cam_ee_1.enable(timestep)
cam_ee_2.enable(timestep)
...
for ep in range(sample_episodes):
    ...
    for steps in range(max_steps):
        ...
        img_ee_1= torch.from_numpy(np.frombuffer(cam_ee_1.getImage(), dtype=np.uint8).reshape(1, 4, 240, 320)).float()
        img_ee_2 = torch.from_numpy(np.frombuffer(cam_ee_2.getImage(), dtype=np.uint8).reshape(1, 4, 320, 240)).float()
        ...
        # write to csv
        os.chdir(base_path)
        list_side = [img_top, img_side, img_ee_1, img_ee_2, current_joint_angles, goal_joint_angles]
        
        with open('2_cam_front_2_cam_ee.csv', 'a') as f_object:
